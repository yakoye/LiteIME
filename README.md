...
当前发布版本：`v0.8.6` · C++20 · Windows TSF
...
- 设置程序的“快捷调用”页以表格统一显示内置和用户入口，列为触发码、候选位置、图标、名称和调用程序，支持添加、编辑、删除及最多 64 项。Windows 工具模板可搜索/分类导入 95 个系统入口和 Everything；模板导入后仍是普通可编辑行。目标支持 EXE、HTML、文件、URL、`ms-settings:`、`cmd:` 和分离程序/参数的 `shell:`。
- 开启英文候选时也可调用这些入口，原始英文固定在候选 1；关闭英文候选时保持逐键直输，需要调用可按 Shift 切回中文。
...
- [v0.8.0 版本说明](docs/release_notes_v0.8.0.md)
- [v0.8.0 验证记录](docs/VERIFICATION_v0.8.0.md)
